"""
Minimal Trellis client: text -> GLB -> `trellis_nvidia/output/`.

Usage:
  uv run python -m trellis_nvidia.text_to_glb --prompt "Toy armored vehicle, olive green"

Notes:
- Hosted endpoint expects prompt length <= 77 characters.
- Reads NVIDIA_API_KEY from environment, then `.env.local`/`.env` in the project root.
"""
